...
IF ros$ = "mac" THEN ERROR 6106
IF ros$ <> "win" AND ros$ <> "lnx" THEN ERROR 6106
...
INPUT #1, versionno$, updaterlinklnx$, updaterlinkwin$, multidownloadlink$, windownloadlink$, linuxdownloadlink$, unziplink$, updatetype, datafolder$, winexe$, lnxexe$, readme$, changelog$, manual$, source$, updatesource$, updatezip$, updateexe$, minsavever$
CLOSE #1
...
IF ros$ = "lnx" THEN SHELL _HIDE "rm *.ddf"
...
IF err = 6101 THEN
	...
	IF ros$ = "lnx" THEN SHELL _HIDE "rm -R vamebackup"
...
IF updatetype = 1 THEN IF lnxexe$ <> "" THEN SHELL _HIDE "copy " + lnxexe$ + " vamebackup\"
...
IF updatetype = 1 THEN IF lnxexe$ <> "" THEN SHELL _HIDE "del " + lnxexe$
...
linuxupdate:
REM updates the game for macos and linux systems
REM backup files
GOSUB displaystep
SHELL _HIDE "mkdir vamebackup"
...
IF updatetype = 1 THEN IF winexe$ <> "" THEN SHELL _HIDE "cp " + winexe$ + " vamebackup/"
...
IF updatetype = 1 THEN IF winexe$ <> "" THEN SHELL _HIDE "rm " + winexe$
